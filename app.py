...
# FIXME: There is an constant / settings needed for this option
ONLY_FLAGGED_PRODUCTS = 'filter=https://online.moysklad.ru/api/remap/1.2/entity/product/metadata/attributes/ccade007' \
                        '-6f68-11eb-0a80-0771002843d1=true '
app.config[
...
def proxy(path):
    ...
    if 'Authorization' in headers_dict:
        ...
        if user is None:
            error_resp = '{"errors":[{"error":"Ошибка аутентификации: Неправильный пароль или имя пользователя или ' \
                         'ключ авторизации","code":1056,' \
                         '"moreInfo":"https://dev.moysklad.ru/doc/api/remap/1.2/#mojsklad-json-api-oshibki"}]} '
            return Response(error_resp, 401)
    else:
        # TODO: encode to JSON from dict
        error_resp = '{"errors":[{"error":"Ошибка аутентификации: Неправильный пароль или имя пользователя или ключ ' \
                     'авторизации","code":1056,' \
                     '"moreInfo":"https://dev.moysklad.ru/doc/api/remap/1.2/#mojsklad-json-api-oshibki"}]} '
        return Response(error_resp, 401)
